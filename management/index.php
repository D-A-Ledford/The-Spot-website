...
if(isset ($_POST['submit']))
{
    ...
    if (empty($formfield['myusername'])){$errormsg .= "<p>*Username is missing.</p>";}
    if (empty($formfield['mypwd'])){$errormsg .= "<div class='error'><p>*Password is missing.</p></div>";}
    ...
    else
    {
        ...
        if ($count < 1)
        {
            $errormsg =  "<div class='error'><p>*The username you entered does not match any account.</p></div>";
        }
        else
        {
            $row = $s->fetch();
			...
			*/
            ...
            if ($count2 <1)
            {
                $errormsg = "<div class='error'><p>*The username and password combination you entered is incorrect.</p></div>";
            }
            else
            {
